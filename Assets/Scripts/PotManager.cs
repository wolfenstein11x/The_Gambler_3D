...
public class PotManager : MonoBehaviour
{
    ...
    {
        DetermineBlindIndexes();
        CollectMoneyFromPlayer(dealer.activePlayers[smallBlindIdx], smallBlind);
        CollectMoneyFromPlayer(dealer.activePlayers[bigBlindIdx], bigBlind);
  
    }

    private void DetermineBlindIndexes()
    {
        // normal case: more than 2 players
        if (dealer.activePlayers.Count > 2)
        {
            // small blind is one space to left of dealer
            smallBlindIdx = dealer.dealerIdx + 1;

            // correct for any mistake caused by wrap-around
            if (smallBlindIdx >= dealer.activePlayers.Count) { smallBlindIdx -= dealer.activePlayers.Count; }

            // big blind is one space to left of small blind
            bigBlindIdx = smallBlindIdx + 1;

            // correct for any mistake caused by wrap-around
            if (bigBlindIdx >= dealer.activePlayers.Count) { bigBlindIdx -= dealer.activePlayers.Count; }
        }

        // heads up case: only 2 players
        else if (dealer.activePlayers.Count == 2)
        {
